...
wd <- "C:/Users/cxc/OneDrive - University of Tasmania/UTAS/Fire/MODIS_global/FIRMS_MODIS/"
setwd(wd)
...
plot_frp_dn <- ggplot(FRP_sum_dn_count, aes(year)) +
  facet_wrap(~dn, labeller = as_labeller(c(`D` = "Day ΣFRP", `N` = "Night ΣFRP"))) +
...
plot_topFRPeventsYearly_meanPoints <- ggplot(FRP_sum_yearly_mean, aes(year, FRP_sum)) +
  ...
  labs(y = "ΣFRP (MW)", title = paste("Mean of top", n_per_year, "\nΣFRP events/year") ) +
...
# read WWF biomes
wwf <- st_read("C:/Users/cxc/OneDrive - University of Tasmania/UTAS/Fire/Global fire disasters/Data/Ecoregions2017/Ecoregions2017.shp")
...
p_ratio_realm <- ggplot(frp_realm_summary, aes(events_to_area, REALM, fill = events_to_area_cat)) +
  ...
  labs(x = "Ratio of major ΣFRP events to realm area", y = element_blank()) +
